...
def flow_to_homography(flow):
    """
    Convierte un flujo BasisHomo (2,h,w) en una matriz 3×3 aproximada.
    Toma desplazamientos de las esquinas y ajusta una homografía.
    """
...
def warp_with_flow(img, flow):
    """
    img: (H,W,C) uint8
    flow: (2,h,w) torch tensor o np.array
    Devuelve img warpeada usando el flujo directly.
    """
...
def stitch_images(img1, img2, H=None, flow=None):
    """
    img1, img2: imágenes uint8
    H: homografía 3×3 (si se usa)
    flow: flujo denso (si se usa)

    Retorna imagen stitched.
    """
...
def basis_homo_stitch(model_output, img1, img2, use_direct_flow=True):
    """
    model_output: salida del Net.forward()
    img1, img2: imágenes originales (uint8)
    use_direct_flow=True → stitching usando flujo denso BasisHomo
    use_direct_flow=False → stitching usando homografía aproximada
    """
